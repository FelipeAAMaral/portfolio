...
import { useEffect, useState } from "react";
import { useLanguage } from "@/context/LanguageContext";
import { client } from "../../tina/__generated__/client"; // ajuste se necessário
  ...
  useEffect(() => {
    async function fetchPosts() {
      try {
        ...
        const posts = res.data.postConnection.edges.map((edge) => {
          const post = edge.node;
          return {
            title: post.title,
            excerpt: post.excerpt || post.body?.children?.[0]?.children?.[0]?.text || "",
            date: post.date,
            image: post.image || "https://source.unsplash.com/random/800x600/?blog",
            url: `/blog/${post._sys.filename}`,
          };
        });
        setBlogPosts(posts);
      } catch (err) {
        console.error("Erro ao carregar posts do Tina:", err);
      } finally {
        setLoading(false);
      }
    }
          ...
          {blogPosts.map((post, idx) => (
            <article key={idx} className="border rounded-lg overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="p-6">
                ...
                <h2 className="text-xl font-semibold mb-3">{post.title}</h2>
                <p className="text-muted-foreground mb-4">{post.excerpt}</p>
                <a href={post.url} className="text-primary hover:underline inline-flex items-center">
                  {t("blog.readMore")}
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 ml-1" viewBox="0 0 20 20" fill="currentColor">
                    <path
                      fillRule="evenodd"
